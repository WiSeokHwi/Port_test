...
public class ConeRangeIndicate : MonoBehaviour
{
    ...
    void Awake() {
        ...
        lr.loop = false;   // 루프값 초기화
        lr.positionCount = segmentCount + 2;  // 시작점 + 세그먼트 + 끝점
    ...
    void DrawCone() {
        ...
        Quaternion leftRotation = Quaternion.Euler(0, -angle / 2f, 0); // -30도~ 30도
        Quaternion rightRotation = Quaternion.Euler(0, angle / 2f, 0);

        points[0] = center; // 중심점 담기

        for (int i = 0; i <= segmentCount; i++) { // 세그먼트 수만큼 반복
            float t = (float)i / segmentCount; // 세그먼트에 따른 i 번째 비율 구하기
            float currentAngle = Mathf.Lerp(-angle / 2f, angle / 2f, t); // 총 각도에 따른 i번째 각도 저장
            Quaternion rotation = Quaternion.Euler(0, currentAngle, 0); // 각도로 변경
            Vector3 dir = rotation * forward; // 방향 저장
            ...
            if (Physics.Raycast(rayOrigin, Vector3.down, out RaycastHit hit, rayHeight * 2, groundMask)) {
                points[i + 1] = new Vector3(hit.point.x, hit.point.y + heightOffset, hit.point.z);
            } else {
                points[i + 1] = new Vector3(horizontalPos.x, center.y + heightOffset, horizontalPos.z);
            }
        }
